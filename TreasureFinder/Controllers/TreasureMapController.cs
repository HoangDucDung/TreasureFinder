...
namespace TreasureFinder.Controllers
{
    ...
    public class TreasureMapController : ControllerBase
    {
        ...
        public async Task<ActionResult<TreasureMapDto>> GetMapById(int id)
        {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<CalculateTreasureRouteResult>> CalculateOptimalRoute(int id)
        {

            var result = await _treasureMapService.CalculateOptimalRouteAsync(id);
            if (result == null)
                return NotFound();
